...
class MiniContoursDownwards(MiniContoursAlgorithm):
    ...
    def get_center_hough_lines(
        ...
        if draw_points:
            ...
            cv2.line(frame, (int(width * (1 - split_factor)), 0),
                     (int(width * (1 - split_factor)), height), self.color3, thickness=2)
        ...
        txt = "angle: " + str(np.round(deg[0], 3)) + " deg " + direction + \
            " x offset: " + str(width // 2 - int(x)) + " pixels"
    ...
    def check_end_of_row(self, frame, points):
        ...
        if len(points) <= 0:
            end_of_row = True
            cv2.putText(frame, "end of row detected", (width // 2, height // 2),
    ...
    def process_frame(self, original_frame, config, show, delta=False):
