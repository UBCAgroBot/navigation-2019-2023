...
class CenterRowAlgorithm:
    ...
    def processFrame(self, frame, show=True):
        ...
        if vanishing_point:
            min_contour, angle = self.get_min_angle(vanishing_point)
            cv.ellipse(black_frame, min_contour, (0, 255, 0), 2)

        return black_frame, vanishing_point

    def get_min_angle(self, vanishing_point):
    ...
    def get_contours(self, binary_mask):
        ...
        # Uses the cv findContours function to find the contours (creates closed shapes around connected pixels) in the image
